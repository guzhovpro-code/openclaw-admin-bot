...
- **💰 Расходы** — расходы на OpenAI API по дням и моделям (опционально, нужен Admin API ключ)
...
| Расходы OpenAI | 30 мин | 🔴 если дневной расход ≥ лимита |
...
# Мониторинг расходов OpenAI (опционально)
OPENAI_ADMIN_KEY=sk-admin-...     # Admin API ключ (не обычный sk-proj-!)
COST_DAILY_LIMIT=10               # лимит расхода в $/день для алерта
```

### Как получить OpenAI Admin API ключ

Для кнопки 💰 Расходы нужен **Admin API ключ** (обычный `sk-proj-...` не подойдёт):

1. Откройте https://platform.openai.com/settings/organization/admin-keys
2. Нажмите «Create new admin key»
3. Скопируйте ключ (начинается с `sk-admin-...`)
4. Добавьте его в `.env` как `OPENAI_ADMIN_KEY`

Если ключ не задан — бот работает без кнопки расходов, всё остальное работает.
